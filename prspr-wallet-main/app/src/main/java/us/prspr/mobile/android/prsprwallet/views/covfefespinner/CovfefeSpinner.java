package com.rndflo.mobile.android.gophrit.views.covfefespinner;
...
public class CovfefeSpinner extends AppCompatTextView {

    private CovfefeSpinner.OnNothingSelectedListener onNothingSelectedListener;
    private CovfefeSpinner.OnItemSelectedListener onItemSelectedListener;
    private CovfefeSpinnerBaseAdapter adapter;
    ...
    public CovfefeSpinner(Context context) {
        super(context);
        init(context, null);
    }

    public CovfefeSpinner(Context context, AttributeSet attrs) {
        super(context, attrs);
        init(context, attrs);
    }

    public CovfefeSpinner(Context context, AttributeSet attrs, int defStyleAttr) {
        super(context, attrs, defStyleAttr);
        init(context, attrs);
    }

    private void init(Context context, AttributeSet attrs) {
        TypedArray ta = context.obtainStyledAttributes(attrs, R.styleable.CovfefeSpinner);
        int defaultColor = getTextColors().getDefaultColor();
        boolean rtl = CooperUtil.isRtl(context);

        try {
            backgroundColor = ta.getColor(R.styleable.CovfefeSpinner_cs_background_color, Color.WHITE);
            textColor = ta.getColor(R.styleable.CovfefeSpinner_cs_text_color, defaultColor);
            arrowColor = ta.getColor(R.styleable.CovfefeSpinner_cs_arrow_tint, textColor);
            hideArrow = ta.getBoolean(R.styleable.CovfefeSpinner_cs_hide_arrow, false);
            popupWindowMaxHeight = ta.getDimensionPixelSize(R.styleable.CovfefeSpinner_cs_dropdown_max_height, 0);
            popupWindowHeight = ta.getLayoutDimension(R.styleable.CovfefeSpinner_cs_dropdown_height, 0);
            arrowColorDisabled = CooperUtil.lighter(arrowColor, 0.8f);
        } finally {
            ta.recycle();
        }
        ...
        left = right = bottom = top = resources.getDimensionPixelSize(R.dimen.covfefe_spinner_item_padding_top);
        if (rtl) {
            right = resources.getDimensionPixelSize(R.dimen.covfefe_spinner_item_padding_left);
        } else {
            left = resources.getDimensionPixelSize(R.dimen.covfefe_spinner_item_padding_left);
        }

        setGravity(Gravity.CENTER_VERTICAL | Gravity.START);
        setClickable(true);
        setPadding(left, top, right, bottom);
        setBackgroundResource(R.drawable.covfefe_spinner_item_selector);
        ...
        if (!hideArrow) {
            arrowDrawable = CooperUtil.getDrawable(context, R.drawable.covfefe_spinner_item_arrow).mutate();
        ...
        listView.setOnItemClickListener(new AdapterView.OnItemClickListener() {
            ...
            public void onItemClick(AdapterView<?> parent, View view, int position, long id) {
                ...
                if (onItemSelectedListener != null) {
                    ...
                    onItemSelectedListener.onItemSelected(CovfefeSpinner.this, position, id, item);
                }
            }
        });
        ...
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.LOLLIPOP) {
            ...
            popupWindow.setBackgroundDrawable(CooperUtil.getDrawable(context, R.drawable.covfefe_spinner_item_drawable));
        } else {
            popupWindow.setBackgroundDrawable(CooperUtil.getDrawable(context, R.drawable.covfefe_spinner_item_drop_down_shadow));
        }
        ...
        popupWindow.setOnDismissListener(new PopupWindow.OnDismissListener() {
            ...
            public void onDismiss() {
                if (nothingSelected && onNothingSelectedListener != null) {
                    onNothingSelectedListener.onNothingSelected(CovfefeSpinner.this);
                }
                if (!hideArrow) {
                    animateArrow(false);
                }
    ...
    public void setBackgroundColor(int color) {
        ...
        if (background instanceof StateListDrawable) { // pre-L
            try {
                ...
                int[] colors = {CooperUtil.darker(color, 0.85f), color};
                for (int i = 0; i < colors.length; i++) {
                    ColorDrawable drawable = (ColorDrawable) getStateDrawable.invoke(background, i);
                    drawable.setColor(colors[i]);
                }
            } catch (Exception e) {
                Log.e("CovfefeSpinner", "Error setting background color", e);
            }
     ...
     */
    public void setOnItemSelectedListener(@Nullable CovfefeSpinner.OnItemSelectedListener onItemSelectedListener) {
     ...
     *     the callback that will run
     */
    public void setOnNothingSelectedListener(@Nullable CovfefeSpinner.OnNothingSelectedListener onNothingSelectedListener) {
    ...
    public <T> void setItems(@NonNull List<T> items) {
        ...
        adapter = new CovfefeSpinnerAdapter<>(getContext(), items).setTextColor(textColor);
    ...
    public void setAdapter(@NonNull ListAdapter adapter) {
        this.adapter = new CovfefeSpinnerAdapterWrapper(getContext(), adapter);
        setAdapterInternal(this.adapter);
    }

    public <T> void setAdapter(CovfefeSpinnerAdapter<T> adapter) {
        this.adapter = adapter;
        setAdapterInternal(adapter);
    }

    private void setAdapterInternal(@NonNull CovfefeSpinnerBaseAdapter adapter) {
    ...
    private int calculatePopupWindowHeight() {
        ...
        float listViewHeight = adapter.getCount() * getResources().getDimension(R.dimen.covfefe_spinner_item_height);
    ...
    public interface OnItemSelectedListener<T> {
         ...
         *     The selected item
         */
        void onItemSelected(CovfefeSpinner view, int position, long id, T item);
    ...
    public interface OnNothingSelectedListener {
         ...
         *
         * @param spinner
         *     the {@link CovfefeSpinner}
         */
        void onNothingSelected(CovfefeSpinner spinner);
    }
